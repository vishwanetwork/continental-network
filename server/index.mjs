  ...
  try {
    ...
    if (req.user && req.query.userId) {
      ...
      for (const agent of result) {
        ...
        // No permissions = visible to all (default)
    ...
    // Attach canEdit flag: only org creator or department leader can edit/delete
    if (req.user && req.query.organizationId) {
      ...
      // If user is org creator or any dept leader, they can edit all visible agents
...
app.post("/api/agents", async (req, res) => {
  try {
    ...
    await pool.execute(
      "INSERT INTO agents (id, name, description, type, input_format, output_format, provider, config) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [id, name.trim(), description || "", type.trim(), inputFormat || "text", outputFormat || "text", provider || "deepseek", config]
    );
...
// Agent share (add a member permission)
app.post("/api/agents/:id/share", async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ error: "Not logged in" });
    const { targetUserId, organizationId } = req.body;
    if (!targetUserId) return res.status(400).json({ error: "targetUserId required" });
    // First check if agent has 'all' permission - if so, sharing is unnecessary
    const [existing] = await pool.execute("SELECT id FROM agent_permissions WHERE agent_id = ? AND target_type = 'all'", [req.params.id]);
    if (existing.length > 0) return res.json({ success: true, message: "Agent is already visible to all" });
    // Add member permission
    await pool.execute(
      "INSERT IGNORE INTO agent_permissions (agent_id, organization_id, target_type, target_id) VALUES (?, ?, 'member', ?)",
      [req.params.id, organizationId || "", targetUserId]
    );
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// ============ Agent Chat API (stateless — no DB storage) ============
app.post("/api/agents/:id/chat", async (req, res) => {
  try {
    ...
    let reply = "";
    if (agent.provider === "deepseek" || agent.provider === "zhipu") {
      const now = new Date();
...
For any other conversation (questions, greetings, how-to), respond normally in the user's language as plain text.`;
      ...
      // Build messages from client-side history (last 10 exchanges max)
      ...
      const aiRes = await fetch("https://api.deepseek.com/chat/completions", {
        ...
        body: JSON.stringify({ model: "deepseek-chat", messages: apiMessages, max_tokens: 16384 }),
      });
      ...
      if (agent.provider === "zhipu") {
        // Try to extract JSON from reply (DeepSeek may wrap it in text or code blocks)
      ...
      reply = `[${agent.name}] Provider ${agent.provider} is not supported for chat.`;
    }
...
app.post("/api/tasks", async (req, res) => {
  try {
    ...
    for (const step of wfSteps) {
      ...
      await pool.execute(
        "INSERT INTO task_steps (task_id, step_order, name, agent_id, status, input, started_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [taskId, step.step_order, step.name, step.agent_id, status, input, status === "running" ? new Date() : null]
      );
    }

    // Start execution asynchronously
    executeTask(taskId).catch((err) => console.error(`Task ${taskId} execution error:`, err));
...
async function executeTask(taskId) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    await pool.execute("UPDATE task_steps SET status = 'running', input = ?, started_at = UTC_TIMESTAMP() WHERE task_id = ? AND step_order = ?", [input, taskId, step.step_order]);

    try {
      // Execute agent
      let output = await executeAgent(step.agent_id, input);

      // FIX LOOP: If this is the tester step, run test→fix cycles (up to 3 rounds)
      if (step.agent_id === "agent-tester" && output && output.includes("--- FILE:")) {
        ...
        for (let round = 0; round < MAX_FIX_ROUNDS; round++) {
          ...
          if (!hasBugs) break; // No more bugs mentioned, stop fixing
          ...
          try {
            const fixedOutput = await executeAgent("agent-developer", fixPrompt);
            if (fixedOutput && fixedOutput.includes("--- FILE:")) {
              // Run tester again on the fixed code
              const retestOutput = await executeAgent("agent-tester", fixedOutput);
      ...
      await pool.execute("UPDATE task_steps SET status = 'completed', output = ?, completed_at = NOW() WHERE task_id = ? AND step_order = ?", [output, taskId, step.step_order]);
    } catch (err) {
      // Mark step as failed
      await pool.execute("UPDATE task_steps SET status = 'failed', error = ?, completed_at = NOW() WHERE task_id = ? AND step_order = ?", [err.message || "Unknown error", taskId, step.step_order]);
      // Mark task as failed
...
async function executeAgent(agentId, input) {
  ...
  if (agent.provider === "deepseek") {
    return await callDeepSeekAgent(config.systemPrompt || "", input);
  } else if (agent.provider === "zhipu") {
...
async function callDeepSeekAgent(systemPrompt, userInput) {
  const now = new Date();
